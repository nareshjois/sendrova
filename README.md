...
**Android:** install `Sendrova-SMS-1.1.0.apk` (or the matching version) via `adb install -r …` (or file manager sideload). Grant SMS (+ Camera for QR). APKs on Releases are **signed** with the project release keystore (see [`apps/android/README.md`](./apps/android/README.md)).
